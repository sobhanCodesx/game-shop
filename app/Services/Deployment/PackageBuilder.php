...
final class PackageBuilder
{
    ...
    public function build(?callable $progress = null): array
    {
        ...
        try {
            ...
            $progress('compressing', 80); $this->zip($stage, $archive);
    ...
    {
        $process = new Process($command, $cwd ?: base_path(), $environment ?: null, timeout: $timeout); $process->run();
    ...
    public function prepareProductionVendor(?callable $progress = null): string
    {
        ...
        try {
            ...
            try {
                $this->copy($project.'/vendor', $cache);
            } catch (\Throwable $e) {
                $this->deleteDirectory($cache);
                throw new RuntimeException('کپی vendor Production به cache ممکن نیست: '.$e->getMessage(), previous: $e);
            }
            if (! is_file($cache.'/autoload.php') || ! is_file($cache.'/composer/installed.php')) {
                $this->deleteDirectory($cache);
                throw new RuntimeException('vendor Production ساخته شد اما خروجی cache ناقص است.');
            }
            $progress('completed', 100);
            return $cache;
    ...
    private function stageStorageStructure(string $stage): void
    {
        foreach ([
            'storage/app/private',
            'storage/app/public',
            ...
            if (! is_dir($path) && ! mkdir($path, 0750, true) && ! is_dir($path)) {
                throw new RuntimeException("ساخت مسیر لازم ممکن نیست: {$directory}");
            }
    ...
    private function copy(string $source, string $target): void
    {
        ...
        foreach ($iterator as $item) {
            ...
            if ($item->isDir()) {
                ...
                $this->stageFile($item->getPathname(), $to);
            }
        }
    }
