...
final class FeedPageDataService
{
    ...
    public function withLiveState(array $pageData, ?User $user): array
    {
        ...
        if ($feedIds->isNotEmpty()) {
            ...
            if ($user) {
                $likedIds = DB::table('social_content_reactions')
                    ->where('user_id', $user->id)->where('type', 'like')
                    ->whereIn('social_content_id', $feedIds)->pluck('social_content_id')
                    ->map(fn ($id) => (int) $id);
                $savedIds = DB::table('social_content_saves')
                    ->where('user_id', $user->id)->whereIn('social_content_id', $feedIds)
                    ->pluck('social_content_id')->map(fn ($id) => (int) $id);
            }
        ...
        if ($videoIds->isNotEmpty()) {
            ...
            $pageData['latestVideos'] = collect($pageData['latestVideos'])->map(function (array $item) use ($views): array {
                $id = (int) ($item['id'] ?? 0);
                return [...$item, 'views' => (int) ($views[$id] ?? 0)];
            })->values()->all();
        }
    ...
    private function build(SocialContent $content): array
    {
        ...
        $latestVideos = SocialContent::query()
            ...
            ->map(fn (array $video) => [...$video, 'views' => 0])
            ->values()->all();
    ...
    private function seoInput(SocialContent $content, array $item, array $breadcrumbs): array
    {
        ...
        $primaryMedia = collect($item['media'])->firstWhere('type', 'image') ?? collect($item['media'])->first();
